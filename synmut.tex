...
modeling indicates that most synonymous mutations in this genomic region
have a selection coefficient of the order of $-0.003$, and that the
majority of synonymous mutations need to be slightly deleterious to be
consistent with the data. This weak selection against synonymous
...
recombination rate between $7\times 10^{-6}- 3\times 10^{-5} 10^{-5}$ by adjusting the
...
quantify, but a substantial fraction of non-synonymous substitutions in gp120 are
...
$k_A=0.005$ per day. We simulate a range from $10^{-4}$ to $10^{-2}$ per day.
...
to its previous cost of -???? with a probability per generation proportional to
...
$k_A$.  The simulations were repeated 1000 times with a seven dimensional
...
frequency $\nu_0$ in panel \ref{fig:fixp2}. The neutral result is
...
patients, we find $P_\text{interm} \approx 0.01$, $A_\text{syn} \approx -0.2$
...
$\alpha$ for which we found $A_\text{syn} < -0.15$ and $0.0025 <
P_\text{interm} < 0.010$. This subset of parameters indicates that in
order to be consistent with the data only
a minority of synonyous sites ($\lesssim 0.2$) can be neutral  and that
...
The selection coefficient $s_d$ tends to be larger than $0.001$, since mutations with smaller
selection coefficients behave neutrally and often go to fixation
...
variants can explain the observated pattern of deleterious synonymous SNVs that
...
non-synonymous mutations are only beneficial until the viral population
...
combined. Again, we found that the rapid rise of several non-synonymous
mutations to intermediate frequencies allows deleterious synonymous
mutations to hitch-hike, while the fixation probability of non-synonymous
...
possibly both important in HIV-1 evolution. Our modeling merely suggest
possible scenarios and to determine to what extend
either of the of these effects contribute to the observed pattern, more
data is required.
...
deleterious effects on the order of $s_d = 0.003$ and that the majority
of all synonymous mutations is deleterious. In absense of hitch-hiking
...
synonymous mutation at high frequency that nevertheless disappears over
a time scale of a year suggests that they are brought to high frequency
by hitch-hiking on favorable genetic backgrounds. 
