...
\begin{figure}
\begin{center}
\subfloat{\includegraphics[width=\linewidth]{Shankarappa_allele_freqs_trajectories_syn_p1.pdf}
\label{fig:aftsyn}}\\
 \subfloat{\includegraphics[width=\linewidth]{Shankarappa_allele_freqs_trajectories_nonsyn_p1.pdf}
\label{fig:aftnonsyn}}
...
\label{fig:fixp2}}
\caption{Fixation and loss of synonymous mutations. \comment{fix x-axis label}
Panel A) Time
...
mutations in C2-V5 are  secondary structures in the viral RNA whose disruption
...
average over all mutations that rare not observed  (green) lies between the
those that fix or get lost.
...
susequently lost (restricted to the region V1-V5$\pm 50bp$)????. This difference
...
logarithmic scale from the indicated windows. \comment{Can't we show this
for simple epitopes? I can't seem to find a way to describe this from the
outset}.
This simulation assumes 6 roughly equivalent escape mutations per epitope; see supplement for alternative models.}
\label{fig:simheat}
\end{center}
\end{figure}
...
known), but our analysis requires longitudinal data most of which focusses
on \env.
Furthermore, we only expect to see deleterious synonymous
mutations at high frequencies if they are linked to repeated adaptive
substitions. Otherwise, they remain at low frequencies and can only be observed
...
influenza evolution~\citep{strelkowa_clonal_2012}. The propagators suggested in
that paper, however, represent ratios between nonsynonymous and synonymous mutations.
...
existing estimates  of the strength of selection
\citep{neher_rate_2010,batorsky_estimate_2011} could
...
materials); a total of 11 patients with 6-?? time points each and 10
sequences per time point were analyzed. The time intervals between two
consecutive sequences ranged from 6 to 18 months. \comment{CHECK VALUES}.
...
criteria for synonymous/nonsynonymous discrimination yield similar results.
...
limit of the resulting curves. Samples less than ???days
prior the last time point were omitted since there was not enough information
on their future fate, i.e., whether they fix or are lost.
...
of the NL4.3 virus used in \citep{watts_architecture_2009}. Problematic
assignments in indel-rich regions are excluded from the analysis.
\comment{The following is super confusing}
In order to restrict the analysis to synonymous polymorphisms, a lower frequency
threshold of 0.15 was used (other thresholds yield the same results). Since very
few polymorphisms hitchhike beyond, say, a frequency of 0.5, this pool is
enriched for to-be-lost mutations; hence the ``lost" curve in \FIG{SHAPEA}
contains much more points than the ``fixed" one.

The variable loops and flanking regions are identified manually starting from
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Simulations are performed using FFPopSim~\citep{zanini_ffpopsim:_2012}. Both full-length HIV genomes and
\env{}-only simulations are performed and yield comparable results. In each
run, a random fitness landscape with specified statistical properties 
(e.g.~density of beneficial sites, average deleterious effect of synonymous changes) is generated.
For each set of parameters, approximately 100 simulation runs are averaged over. 
\comment{needs a little bit more detail}.
