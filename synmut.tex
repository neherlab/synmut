...
outset}. \comment{FIXME: (i) only x acis is logarithmic; (ii) redo without
epitopes (less representative in the end, but easier to introduce).}
...
known), but our analysis requires longitudinal data, most of which focuses
on \env{}.
...
consecutive sequences ranged from 1 to 34 months, most of them approximately 6 to 10 months.
...
deleterious effect at those sites (logarithmic between $10^{-4}$ and $10^{-2}$);
(iii) average beneficial effect of an escape mutation (logarithmic between
$10^{-2.5}$ and $10^{-1.5}$); (iv) rate of introduction of new epitopes
(logarithmic between $10^{-3}$ and $10^{-2}$ per generation). After these four
...
results), and no epitopes (i.e. no escape mutations) were initially present.
After 30 generations of burn-in to create some genetic diversity, new epitopes
...
reverted at random after it had appeared, according to its integrated allele
