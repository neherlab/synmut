...
\begin{figure}
\begin{center}
\subfloat{\includegraphics[width=0.9\linewidth]{reactivities_histograms_syn}
\label{fig:SHAPEA}}\\
...
In the three simulations shown, the fixation probability of synonymous alleles
...
(\FIG{simfixpvar}, from $-0.0003$ in blue to $-0.003$ in red).

Among all simulations, we select the ones that show HIV-like $A_\text{syn}$ and
$P_\text{interm}$, i.e. a large depression in
fixation probability of synonymous mutations but, simulteneously, a moderately
high synonymous diversity: simulations are gated such that $A_\text{syn} < -0.15$
and $0.0025 < P_\text{interm} < 0.010$, hence allowing an uncertainty on mutation
rates or generation times of about twofold. This selection imposes a strong
